...
| `test_abas.py` | Dois arquivos abertos não compartilham texto, o autosave grava no que foi editado mesmo depois de trocar de aba, dois terminais são sessões distintas, fechar um não derruba o outro, as duas listas de abas voltam depois do reload, selecionar no terminal já copia sem que o Ctrl+C deixe de interromper, e OSC 52 (Codex `/copy`) cai no clipboard. |
